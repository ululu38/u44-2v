    ...
    <div className="relative mb-12 group" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
      ...
      >
        {images.map((img, index) => {
          const mediaObj = img.media || img;
          const src = getImageUrl(mediaObj.urlFull);
          return (
          <div 
            key={`${mediaObj.id || index}-${index}`} 
            className="flex-shrink-0 h-full snap-center relative flex items-center justify-center transition-all duration-500"
            style={{ 
          ...
          >
            ...
            <img
              ...
              className="h-full w-auto max-w-none shadow-2xl object-contain block"
              style={{ display: 'block' }}
            />
          </div>
        )})}
