...
export default function ClientGroupsPage() {
  ...
  const fetchGroups = async (page = 1, reset = false) => {
    setLoading(true);
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/client-groups?page=${page}&limit=10`, {
        credentials: 'include'
      });
      if (response.ok) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      if (response.ok) fetchGroups(1, true);
    } catch (err) {
      ...
      <div className="space-y-6 animate-in fade-in duration-500">
        <div className="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          ...
          >
            ...
            <span className="hidden min-[500px]:inline">New Group</span>
          </button>
        </div>
