...
export default function PartnersPage() {
  ...
  const fetchPartners = async (page = 1, reset = false) => {
    setLoading(true);
    try {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      if (response.ok) fetchPartners(1, true);
    } catch (err) {
    ...
    <>
      ...
      <div className="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        ...
        >
          ...
          <span className="hidden min-[500px]:inline">New Partner</span>
        </button>
      </div>
              ...
              {filteredPartners.map((p) => (
                <tr key={p.partnerId} className="hover:bg-blue-50/30 transition-colors group">
                  <td className="px-6 py-5">
                    <div className="flex flex-col min-[500px]:flex-row min-[500px]:items-center items-start gap-2.5 min-[500px]:gap-4">
                      <div className="w-14 h-14 rounded-lg overflow-hidden border border-gray-200 bg-white p-1 shrink-0">
                        {p.logoMedia ? (
                          <CachedImage
                            src={(p.logoMedia.urlMini || p.logoMedia.urlThumb || p.logoMedia.urlFull).startsWith('/') && !(p.logoMedia.urlMini || p.logoMedia.urlThumb || p.logoMedia.urlFull).startsWith('/images/') ? `${process.env.NEXT_PUBLIC_IMAGE_URL}${p.logoMedia.urlMini || p.logoMedia.urlThumb || p.logoMedia.urlFull}` : (p.logoMedia.urlMini || p.logoMedia.urlThumb || p.logoMedia.urlFull)}
                            alt=""
                      ...
                      <span 
                        className="text-base font-bold text-gray-800 group-hover:text-blue-600 transition-colors leading-tight line-clamp-2"
                        title={p.name}
                      >
                        {p.name}
                      </span>
