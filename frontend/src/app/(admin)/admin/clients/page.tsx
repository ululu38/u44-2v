...
export default function ClientsPage() {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      if (response.ok) fetchClients(1, true);
    } catch (err) {
  ...
  return (
    <>
      <div className="space-y-6 animate-in fade-in duration-500">
        <div className="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          ...
          >
            ...
            <span className="hidden min-[500px]:inline">New Client</span>
          </button>
        </div>
      ...
      {isModalOpen && mounted && createPortal(
        <div className="fixed inset-0 z-50 flex justify-center p-4 bg-black/40 backdrop-blur-sm overflow-y-auto py-10" onClick={() => setIsModalOpen(false)}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-xl w-full p-8 my-auto animate-in zoom-in-95 duration-300" onClick={(e) => e.stopPropagation()}>
