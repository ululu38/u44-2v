...
class PostRenderer {
  ...
  addHoverEffect() {
    ...
    const handleMouseEnter = (col, index, allCols) => {
      ...
      allCols.forEach((otherCol, otherIndex) => {
        const otherRowIndex = Math.floor(otherIndex / 4);
        const otherColumnIndex = otherIndex % 4;
    ...
    const handleMouseLeave = (allCols) => {
    ...
    document.querySelectorAll(".col-3-extra").forEach((col, index, allCols) => {
      col.addEventListener(
        "mouseenter",
        debounce(() => handleMouseEnter(col, index, allCols), 100)
      );
      col.addEventListener(
        "mouseleave",
        debounce(() => handleMouseLeave(allCols), 100)
      );
    });
  }
  ...
  renderPagination(currentPage, totalPages, onPageChange) {
    ...
    this.pagination.insertAdjacentHTML(
      ...
      `<li class="page-item ${prevClass}">
                <a class="page-link btn btn-primary rounded-pill d-flex align-items-center text-white link-nounder" href="#" aria-label="Previous" data-page="${currentPage - 1
      }">
    ...
    for (let i = 1; i <= totalPages; i++) {
      if (totalPages > 7 && i !== currentPage) continue;
    ...
    this.pagination.insertAdjacentHTML(
      ...
      `<li class="page-item ${nextClass}">
                <a class="page-link btn btn-primary rounded-pill d-flex align-items-center text-white link-nounder" href="#" aria-label="Next" data-page="${currentPage + 1
      }">
                    <div>Next</div>
...
(async () => {
  ...
  const renderer = new PostRenderer("movement", "pagination");
...
//                         <div class="card h-100 border-0">
//                             <a href="?pages=article&act=post&post=${
//                               post.post_id
//                             }" class="link-nounder">
//                                 <img src="${
//                                   post.thumbnail_path
...
//         post.title
//       }" style="width:300px;height:180px;" loading="lazy" />
//                                 <div class="card-body">
//                                     <h5 class="card-title text-truncate">${
//                                       post.title
//                                     }</h5>
//                                     <p class="card-text text-truncate-2">${this.stripHTML(
//                                       post.content
//                                     )}</p>
//                                     <p class="card-text"><small class="text-body-secondary">${formattedDate}</small></p>
//                                 </div>
