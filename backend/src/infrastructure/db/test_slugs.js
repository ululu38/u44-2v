...
async function main() {
  ...
  console.log(`\n🔍 Testing ${posts.length} post slugs against ${API}\n`);
  ...
  for (const post of posts) {
    ...
    // ─── Step 2: Decode (same as frontend) ─────────────────────────────────
    const parts = slug.split('-');
    const encodedId = parts.pop();
    const ids = sqids.decode(encodedId);
    ...
    if (!ids.length) {
      fail++;
      failures.push({ postId: post.post_id, reason: `sqids.decode("${encodedId}") → []`, slug });
      continue;
    }
    ...
    if (decodedId !== post.post_id) {
      ...
      failures.push({
        postId: post.post_id,
        reason: `decoded ID ${decodedId} ≠ actual postId ${post.post_id}`,
        slug,
      });
      continue;
    }

    // ─── Step 3: Hit the API (optional — checks backend is alive) ──────────
    ...
    if (fetched.postId !== post.post_id) {
      ...
      failures.push({
        postId: post.post_id,
        reason: `API returned postId ${fetched.postId} ≠ ${post.post_id}`,
        slug,
      });
      continue;
    }
    ...
    pass++;
    console.log(`  ✅ [${String(post.post_id).padStart(3)}] ${slug}`);
  }

  console.log('─'.repeat(80));

  if (warnings.length) {
    console.log(`\n⚠️  WARNINGS (API unreachable / no data):`);
