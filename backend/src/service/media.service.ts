...
import { media, mediaBlobs } from '../infrastructure/db/schema.js';
...
export function toUrl(filename: string): string {
  ...
  const bare = filename.replace(/^\/+/, '').replace(/^(?:media\/)?uploads\/(?:thumbnails\/)?/, '');
...
export class MediaService {
  ...
  async remove(id: number) {
    ...
    await this.drizzle.db.delete(mediaBlobs).where(eq(mediaBlobs.id, id));
    await this.drizzle.db.delete(media).where(eq(media.id, id));
    return item;
  }

  async getFileBuffer(filename: string): Promise<{ buffer: Buffer; contentType: string } | null> {
    let baseName = filename;
    let sizeType: 'full' | 'thumb' | 'mini' | 'original' = 'full';
    ...
    if (filename.endsWith('-full.webp')) {
      ...
      const originalMatch = filename.match(/^(.+)-original(\.[a-zA-Z0-9]+)$/);
      if (originalMatch) {
        baseName = originalMatch[1];
        sizeType = 'original';
      }
    }
    ...
    const mediaItem = await this.drizzle.db.query.media.findFirst({
      where: eq(media.filename, baseName),
    ...
    // Only fetch the specific column we need to save memory and DB bandwidth
    if (sizeType === 'full') {
      const row = await this.drizzle.db.select({ data: mediaBlobs.dataFull }).from(mediaBlobs).where(eq(mediaBlobs.id, mediaItem.id)).limit(1);
      buffer = row[0]?.data ?? null;
    } else if (sizeType === 'thumb') {
      const row = await this.drizzle.db.select({ data: mediaBlobs.dataThumb }).from(mediaBlobs).where(eq(mediaBlobs.id, mediaItem.id)).limit(1);
      buffer = row[0]?.data ?? null;
    } else if (sizeType === 'mini') {
      const row = await this.drizzle.db.select({ data: mediaBlobs.dataMini }).from(mediaBlobs).where(eq(mediaBlobs.id, mediaItem.id)).limit(1);
      buffer = row[0]?.data ?? null;
    } else if (sizeType === 'original') {
      const row = await this.drizzle.db.select({ data: mediaBlobs.dataOriginal }).from(mediaBlobs).where(eq(mediaBlobs.id, mediaItem.id)).limit(1);
      buffer = row[0]?.data ?? null;
