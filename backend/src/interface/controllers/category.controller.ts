import { Controller, Get, Post, Patch, Delete, Body, Param, UseGuards, ParseIntPipe } from '@nestjs/common';
import { ApiTags, ApiOperation, ApiResponse, ApiCookieAuth } from '@nestjs/swagger';
import { DrizzleService } from '../../infrastructure/db/drizzle.service.js';
import { categories } from '../../domain/entities/schema.js';
import { eq, count } from 'drizzle-orm';
import { JwtAuthGuard } from '../../infrastructure/auth/guards/jwt-auth.guard.js';
import { RolesGuard } from '../../infrastructure/auth/guards/roles.guard.js';
import { Roles } from '../../infrastructure/auth/decorators/roles.decorator.js';
import { CreateCategoryDto, UpdateCategoryDto } from '../dtos/category.dto.js';
import { Query } from '@nestjs/common';

@ApiTags('categories')
@Controller('categories')
export class CategoryController {
  constructor(private readonly drizzle: DrizzleService) {}

  @Post()
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('admin', 'employee')
  @ApiCookieAuth()
  @ApiOperation({ summary: 'Create a new category' })
  async create(@Body() createCategoryDto: CreateCategoryDto) {
    const result = await this.drizzle.db.insert(categories).values(createCategoryDto).returning();
    return result[0];
  }

  @Get()
  @ApiOperation({ summary: 'Get all categories' })
  async findAll(
    @Query('page') page: string = '1',
    @Query('limit') limit: string = '10'
  ) {
    const p = parseInt(page);
    const l = parseInt(limit);
    const offset = (p - 1) * l;

    const data = await this.drizzle.db.query.categories.findMany({
      limit: l,
      offset: offset,
    });

    const totalCount = await this.drizzle.db.select({ value: count() }).from(categories);

    return {
      data,
      meta: {
        total: totalCount[0].value,
        page: p,
        limit: l,
        totalPages: Math.ceil(totalCount[0].value / l),
      }
    };
  }

  @Patch(':id')
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('admin', 'employee')
  @ApiCookieAuth()
  @ApiOperation({ summary: 'Update a category' })
  async update(@Param('id', ParseIntPipe) id: number, @Body() updateCategoryDto: UpdateCategoryDto) {
    const result = await this.drizzle.db.update(categories)
      .set(updateCategoryDto)
      .where(eq(categories.categoryId, id))
      .returning();
    return result[0];
  }

  @Delete(':id')
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('admin', 'employee')
  @ApiCookieAuth()
  @ApiOperation({ summary: 'Delete a category' })
  async remove(@Param('id', ParseIntPipe) id: number) {
    const result = await this.drizzle.db.delete(categories)
      .where(eq(categories.categoryId, id))
      .returning();
    return result[0];
  }
}
