...
async function main() {
  console.log("ðŸš€ Starting migration of partners...");
  const pool = new Pool({
  ...
  for (let i = 0; i < partnersData.length; i++) {
    ...
    try {
      ...
      if (existing) {
        console.log(`â­ï¸ Skipped (already exists): ${partner.name}`);
        continue;
      }
      ...
      console.log(`âœ… Migrated: ${partner.name} (Logo: ${partner.logo})`);
      count++;
    } catch (err: any) {
      console.error(`âŒ Failed to migrate ${partner.name} (${partner.logo}):`, err.message);
    }
  }

  console.log(`ðŸŽ‰ Finished! Successfully migrated ${count}/${partnersData.length} partners.`);
  await pool.end();
}
