...
async function getPostForMetadata(slug: string) {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/posts/${ids[0]}`, {
    headers,
    next: {
      revalidate: 3600,
  ...
  return (
    <main className="container mx-auto px-4 py-12 max-w-4xl min-h-screen">
      ...
      <article className="mt-8 md:mt-16 animate-in fade-in slide-in-from-bottom-8 duration-700">
        ...
        <SafeHtmlRenderer 
          html={post.content} 
          className="prose prose-invert prose-lg max-w-none prose-img:rounded-xl prose-a:text-blue-400 hover:prose-a:text-blue-300 mt-12"
        />
      </article>
    </main>
  );
