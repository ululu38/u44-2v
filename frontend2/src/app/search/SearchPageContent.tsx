    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row gap-3 max-w-2xl mx-auto items-stretch">
        <SearchBar
          ...
          placeholder="พิมพ์คำค้นหา เช่น NestJS, Design, Google..."
          className="flex-1"
          inputClassName="!bg-neutral-900 !border-neutral-700 !text-neutral-100 !rounded-xl !pl-4 !pr-12 !py-3 focus:!border-neutral-500/50 transition-all placeholder-neutral-500 !text-base"
          onSearch={(val) => {
            setInputValue(val);
            setKeyword(val);
            const params = new URLSearchParams(window.location.search);
        ...
        {meta && (
          <div className="flex items-center justify-center shrink-0 px-5 py-3 bg-neutral-900 border border-neutral-700 rounded-xl text-sm text-neutral-400">
            <span className="text-white font-bold mr-1">{meta.total}</span>
            รายการ
          </div>
        )}
        ...
        {meta && (
          <button
            ...
            className="px-3 py-3 bg-neutral-800 hover:bg-neutral-700 text-neutral-400 rounded-xl transition-colors shrink-0 flex items-center justify-center cursor-pointer"
      ...
      {/* ── Suspense boundary: first-page skeleton ────────────────────────── */}
